...
app.config['MONGO_URI'] = "mongodb://localhost:27017/firstdbapi" 
mongo = PyMongo(app)
...
def addrecord():
    ...
    if name and email and password and request.method == 'POST':
        ...
        id = mongo.db.first.insert_one(datamap)
...
def deletedocument(id):
    mongo.db.first.delete_one({'_id':ObjectId(id)})
...
def viewrecordbyid(id):
    data = mongo.db.first.find_one({'_id':ObjectId(id)})
    data = dumps(data)
    return data
...
def recordbyidusingPost():
    ...
    if id and request.method == 'POST':
        resp = mongo.db.first.find_one({'_id':ObjectId(id)})
        resp = dumps(resp)
...
def updateRecord(id):
    ...
    if name and email and password and request.method == 'PUT':
        ...
        mongo.db.first.update_one({'_id':ObjectId(id['$oid']) if '$oid' in id else ObjectId(id)},{'$set':updateddatamap})
